...
public class WeatherGenerator {
    ...
    public static int[] oneMonthGenerator(double longitude, double latitude, int month, double[][] drywet, double[][] wetwet) {
        double dryWetProb = 0.0;
        double wetWetProb = 0.0;
        int monthLength = numberOfDaysInMonth[month - 2];
        int[] monthForecast = new int[monthLength];
        
        for(int i = 0; i < drywet.length; i++) {
            if(drywet[i][0] == longitude && drywet[i][1] == latitude) {
                dryWetProb = drywet[i][month];
            }
        }
        for(int j = 0; j < wetwet.length; j++) {
            if(drywet[j][0] == longitude && drywet[j][1] == latitude) {
                wetWetProb = wetwet[j][month];
            }
        }
        
        int prevDay = WET;
        double subprob = StdRandom.uniform();
        if ( subprob < 0.5 ){
            prevDay = WET;
        } else {
            prevDay = DRY;
        ...
        monthForecast[0] = prevDay;
        
        for(int k = 1; k < monthLength; k++) {
            subprob = StdRandom.uniform();

            if(prevDay == WET) {
         
               if(subprob <= wetWetProb) {
                   monthForecast[k] = WET;
               } else {
                   monthForecast[k] = DRY;
               }
               ...
               if(subprob <= dryWetProb) {
                   monthForecast[k] = WET;
               } else {
                   monthForecast[k] = DRY;
               }
            }
            prevDay = monthForecast[k];
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int i = 1; i < forecast.length; i++) {
            if ( ( forecast[i] == forecast[i-1] ) && ((forecast[i] == mode)) ){
                count++;
                if ( maxcount < count ) {
                    maxcount = count;
                }
    ...
    public static void main (String[] args) {
        ...
        //long    epoch     = Long.parseLong(args[3]);
        //StdRandom.setSeed(epoch);
